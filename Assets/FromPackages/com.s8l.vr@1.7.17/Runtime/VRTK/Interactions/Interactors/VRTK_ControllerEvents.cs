...
namespace VRTK
{
    ...
    public class VRTK_ControllerEvents : MonoBehaviour
    {
        ...
        [Tooltip(
            "The amount of fidelity in the changes on the axis, which is defaulted to 1. Any number higher than 2 will probably give too sensitive results.")]
        public int axisFidelity = 1;

        [Tooltip("The level on a sense axis to reach before the sense axis is forced to 0f")] [Range(0f, 1f)]
        public float senseAxisForceZeroThreshold = 0.15f;

        [Tooltip(
            "The amount of pressure required to be applied to a sense button before considering the sense button pressed.")]
        [Range(0f, 1f)]
        ...
        [Tooltip(
            "If this is checked then the trigger axis will be forced to 0f when the trigger button reports an untouch event.")]
        public bool triggerAxisZeroOnUntouch = false;

        [Tooltip("The level on the trigger axis to reach before a hairline is registered.")]
        public float triggerHairlineThreshold = 0.1f;

        [Tooltip("The level on the trigger axis to reach before a press is registered.")]
        ...
        [Tooltip(
            "If this is checked then the grip axis will be forced to 0f when the grip button reports an untouch event.")]
        ...
        /// </summary>
        [HideInInspector] public bool triggerSenseAxisChanged = false;
        ...
        /// </summary>
        [HideInInspector] public bool touchpadSenseAxisChanged = false;

        /// <summary>
        /// This will be true if the touchpad two is being touched.
        /// </summary>
        [HideInInspector] public bool touchpadTwoTouched = false;

        /// <summary>
        /// This will be true if the touchpad two is held down.
        /// </summary>
        [HideInInspector] public bool touchpadTwoPressed = false;

        /// <summary>
        /// This will be true if the touchpad two position has changed.
        /// </summary>
        [HideInInspector] public bool touchpadTwoAxisChanged = false;
        ...
        /// </summary>
        [HideInInspector] public bool middleFingerSenseAxisChanged = false;

        /// <summary>
        /// This will be true if the ring finger sense is being touched more or less.
        /// </summary>
        [HideInInspector] public bool ringFingerSenseAxisChanged = false;

        /// <summary>
        /// This will be true if the pinky finger sense is being touched more or less.
        /// </summary>
        [HideInInspector] public bool pinkyFingerSenseAxisChanged = false;

        /// <summary>
        /// This will be true if the grip sense is being touched more or less.
        /// </summary>
        [HideInInspector] public bool gripSenseAxisChanged = false;

        /// <summary>
        /// This will be true if grip sense is held down.
        /// </summary>
        [HideInInspector] public bool gripSensePressed = false;
        ...
        /// <returns>The payload for a Controller Event.</returns>
        public virtual ControllerInteractionEventArgs SetControllerEvent(ref bool buttonBool, bool value = false,
            float buttonPressure = 0f)
        {
            ...
            e.touchpadTwoAxis =
                VRTK_SDK_Bridge.GetControllerAxis(SDK_BaseController.ButtonTypes.TouchpadTwo, controllerReference);
        ...
        {
            return (trackedController != null
                ? trackedController.GetControllerType()
        ...
        {
            return (triggerPressed || gripPressed || touchpadPressed || touchpadTwoPressed || buttonOnePressed ||
                    buttonTwoPressed || startMenuPressed || gripSensePressed);
        }
        ...
        public virtual void SubscribeToButtonAliasEvent(ButtonAlias givenButton, bool startEvent,
        ...
        public virtual void UnsubscribeToButtonAliasEvent(ButtonAlias givenButton, bool startEvent,
            ControllerInteractionEventHandler callbackMethod)
        {
        ...
        /// <param name="callbackMethod">The method to subscribe to the event.</param>
        public virtual void SubscribeToAxisAliasEvent(SDK_BaseController.ButtonTypes buttonType, AxisType axisType,
            ControllerInteractionEventHandler callbackMethod)
        {
        ...
        /// <param name="callbackMethod">The method to unsubscribe from the event.</param>
        public virtual void UnsubscribeToAxisAliasEvent(SDK_BaseController.ButtonTypes buttonType, AxisType axisType,
            ControllerInteractionEventHandler callbackMethod)
        {
        ...
        protected virtual void CheckTriggerEvents(VRTK_ControllerReference controllerReference)
        {
            ...
            float currentTriggerSenseAxis =
                ProcessSenseAxis(VRTK_SDK_Bridge.GetControllerSenseAxis(SDK_BaseController.ButtonTypes.Trigger,
                    controllerReference));

            //Trigger Touched
            if (VRTK_SDK_Bridge.GetControllerButtonState(SDK_BaseController.ButtonTypes.Trigger,
            ...
            //Trigger Hairline
            if (!triggerHairlinePressed && currentTriggerAxis.x >= triggerHairlineThreshold)
            {
                OnTriggerHairlineStart(SetControllerEvent(ref triggerHairlinePressed, true, currentTriggerAxis.x));
            }

            //Trigger Pressed
            if (!triggerPressed && currentTriggerAxis.x >= triggerPressThreshold)
            {
            ...
            //Trigger Touch End
            if (VRTK_SDK_Bridge.GetControllerButtonState(SDK_BaseController.ButtonTypes.Trigger,
                SDK_BaseController.ButtonPressTypes.TouchUp, controllerReference))
            {
            ...
            currentTriggerAxis.x =
                ((!triggerTouched && triggerAxisZeroOnUntouch) || currentTriggerAxis.x < triggerForceZeroThreshold
                    ? 0f
                    : currentTriggerAxis.x);
            ...
            //Trigger Sense Axis
            if (VRTK_SharedMethods.RoundFloat(triggerSenseAxis, axisFidelity) ==
                VRTK_SharedMethods.RoundFloat(currentTriggerSenseAxis, axisFidelity))
            {
                triggerSenseAxisChanged = false;
            }
            else
            {
                OnTriggerSenseAxisChanged(
                    SetControllerEvent(ref triggerSenseAxisChanged, true, currentTriggerSenseAxis));
            }
            ...
            hairTriggerDelta =
                VRTK_SDK_Bridge.GetControllerHairlineDelta(SDK_BaseController.ButtonTypes.TriggerHairline,
                    controllerReference);
        }
        ...
        protected virtual void CheckGripEvents(VRTK_ControllerReference controllerReference)
        {
            ...
            //Grip Touched
            if (VRTK_SDK_Bridge.GetControllerButtonState(SDK_BaseController.ButtonTypes.Grip,
                SDK_BaseController.ButtonPressTypes.TouchDown, controllerReference))
            {
            ...
            //Grip Hairline
            if (VRTK_SDK_Bridge.GetControllerButtonState(SDK_BaseController.ButtonTypes.GripHairline,
                SDK_BaseController.ButtonPressTypes.PressDown, controllerReference))
            {
            ...
            // Grip Pressed End
            if (VRTK_SDK_Bridge.GetControllerButtonState(SDK_BaseController.ButtonTypes.Grip,
                SDK_BaseController.ButtonPressTypes.PressUp, controllerReference))
            {
            ...
            //Grip Axis
            currentGripAxis.x = ((!gripTouched && gripAxisZeroOnUntouch) || currentGripAxis.x < gripForceZeroThreshold
                ? 0f
                : currentGripAxis.x);
            ...
            hairGripDelta =
                VRTK_SDK_Bridge.GetControllerHairlineDelta(SDK_BaseController.ButtonTypes.GripHairline,
                    controllerReference);
        }
        ...
        protected virtual void CheckTouchpadEvents(VRTK_ControllerReference controllerReference)
        {
            Vector2 currentTouchpadAxis =
                VRTK_SDK_Bridge.GetControllerAxis(SDK_BaseController.ButtonTypes.Touchpad, controllerReference);
            float currentTouchpadSenseAxis =
                ProcessSenseAxis(VRTK_SDK_Bridge.GetControllerSenseAxis(SDK_BaseController.ButtonTypes.Touchpad,
                    controllerReference));

            //Touchpad Touched
            if (VRTK_SDK_Bridge.GetControllerButtonState(SDK_BaseController.ButtonTypes.Touchpad,
                SDK_BaseController.ButtonPressTypes.TouchDown, controllerReference))
            {
            ...
            else if (VRTK_SDK_Bridge.GetControllerButtonState(SDK_BaseController.ButtonTypes.Touchpad,
                SDK_BaseController.ButtonPressTypes.PressUp, controllerReference))
            {
            ...
            //Touchpad Sense Axis
            if (VRTK_SharedMethods.RoundFloat(touchpadSenseAxis, axisFidelity) ==
                VRTK_SharedMethods.RoundFloat(currentTouchpadSenseAxis, axisFidelity))
            {
                touchpadSenseAxisChanged = false;
            }
            else
            {
                OnTouchpadSenseAxisChanged(SetControllerEvent(ref touchpadSenseAxisChanged, true,
                    currentTouchpadSenseAxis));
            }

            touchpadAxis = (touchpadAxisChanged
                ? new Vector2(currentTouchpadAxis.x, currentTouchpadAxis.y)
                : touchpadAxis);
        ...
        {
            Vector2 currentTouchpadTwoAxis =
                VRTK_SDK_Bridge.GetControllerAxis(SDK_BaseController.ButtonTypes.TouchpadTwo, controllerReference);

            //Touchpad Two Touched
            if (VRTK_SDK_Bridge.GetControllerButtonState(SDK_BaseController.ButtonTypes.TouchpadTwo,
                SDK_BaseController.ButtonPressTypes.TouchDown, controllerReference))
            {
            ...
            else if (VRTK_SDK_Bridge.GetControllerButtonState(SDK_BaseController.ButtonTypes.TouchpadTwo,
                SDK_BaseController.ButtonPressTypes.PressUp, controllerReference))
            {
            ...
            touchpadTwoAxis = (touchpadTwoAxisChanged
                ? new Vector2(currentTouchpadTwoAxis.x, currentTouchpadTwoAxis.y)
                : touchpadTwoAxis);
        }
        ...
        {
            //ButtonOne Touched
            if (VRTK_SDK_Bridge.GetControllerButtonState(SDK_BaseController.ButtonTypes.ButtonOne,
                SDK_BaseController.ButtonPressTypes.TouchDown, controllerReference))
            {
            ...
            else if (VRTK_SDK_Bridge.GetControllerButtonState(SDK_BaseController.ButtonTypes.ButtonOne,
                SDK_BaseController.ButtonPressTypes.PressUp, controllerReference))
            {
        ...
        {
            //ButtonTwo Touched
            if (VRTK_SDK_Bridge.GetControllerButtonState(SDK_BaseController.ButtonTypes.ButtonTwo,
                SDK_BaseController.ButtonPressTypes.TouchDown, controllerReference))
            {
            ...
            else if (VRTK_SDK_Bridge.GetControllerButtonState(SDK_BaseController.ButtonTypes.ButtonTwo,
                SDK_BaseController.ButtonPressTypes.PressUp, controllerReference))
            {
        ...
        {
            //StartMenu Pressed
            if (VRTK_SDK_Bridge.GetControllerButtonState(SDK_BaseController.ButtonTypes.StartMenu,
                SDK_BaseController.ButtonPressTypes.PressDown, controllerReference))
            {
                OnStartMenuPressed(SetControllerEvent(ref startMenuPressed, true, 1f));
            }
            else if (VRTK_SDK_Bridge.GetControllerButtonState(SDK_BaseController.ButtonTypes.StartMenu,
                SDK_BaseController.ButtonPressTypes.PressUp, controllerReference))
            {
        ...
        {
            float currentMiddleFingerSenseAxis =
                ProcessSenseAxis(VRTK_SDK_Bridge.GetControllerSenseAxis(SDK_BaseController.ButtonTypes.MiddleFinger,
                    controllerReference));
            float currentRingFingerSenseAxis =
                ProcessSenseAxis(VRTK_SDK_Bridge.GetControllerSenseAxis(SDK_BaseController.ButtonTypes.RingFinger,
                    controllerReference));
            float currentPinkyFingerSenseAxis =
                ProcessSenseAxis(VRTK_SDK_Bridge.GetControllerSenseAxis(SDK_BaseController.ButtonTypes.PinkyFinger,
                    controllerReference));
            float currentGripSenseAxis =
                ProcessSenseAxis(VRTK_SDK_Bridge.GetControllerSenseAxis(SDK_BaseController.ButtonTypes.Grip,
                    controllerReference));

            //Middle Finger Sense Axis
            if (VRTK_SharedMethods.RoundFloat(middleFingerSenseAxis, axisFidelity) ==
                VRTK_SharedMethods.RoundFloat(currentMiddleFingerSenseAxis, axisFidelity))
            {
                middleFingerSenseAxisChanged = false;
            }
            else
            {
                OnMiddleFingerSenseAxisChanged(SetControllerEvent(ref middleFingerSenseAxisChanged, true,
                    currentMiddleFingerSenseAxis));
            }

            //Ring Finger Sense Axis
            if (VRTK_SharedMethods.RoundFloat(ringFingerSenseAxis, axisFidelity) ==
                VRTK_SharedMethods.RoundFloat(currentRingFingerSenseAxis, axisFidelity))
            {
                ringFingerSenseAxisChanged = false;
            }
            else
            {
                OnRingFingerSenseAxisChanged(SetControllerEvent(ref ringFingerSenseAxisChanged, true,
                    currentRingFingerSenseAxis));
            }

            //Pinky Finger Sense Axis
            if (VRTK_SharedMethods.RoundFloat(pinkyFingerSenseAxis, axisFidelity) ==
                VRTK_SharedMethods.RoundFloat(currentPinkyFingerSenseAxis, axisFidelity))
            {
                pinkyFingerSenseAxisChanged = false;
            }
            else
            {
                OnPinkyFingerSenseAxisChanged(SetControllerEvent(ref pinkyFingerSenseAxisChanged, true,
                    currentPinkyFingerSenseAxis));
            }
            ...
            //Grip Sense Axis
            if (VRTK_SharedMethods.RoundFloat(gripSenseAxis, axisFidelity) ==
                VRTK_SharedMethods.RoundFloat(currentGripSenseAxis, axisFidelity))
            {
            ...
            middleFingerSenseAxis =
                (middleFingerSenseAxisChanged ? currentMiddleFingerSenseAxis : middleFingerSenseAxis);
        ...
        protected virtual void ButtonAliasEventSubscription(bool subscribe, ButtonAlias givenButton, bool startEvent,
            ControllerInteractionEventHandler callbackMethod)
        {
            switch (givenButton)
            {
        ...
        protected virtual void AxisAliasEventSubscription(bool subscribe, SDK_BaseController.ButtonTypes buttonType,
            AxisType axisType, ControllerInteractionEventHandler callbackMethod)
        {
            switch (buttonType)
            {
        ...
        protected virtual void DisableEvents()
        {
            ...
            if (VRTK_ControllerReference.IsValid(controllerReference))
            {
                ...
                hairTriggerDelta =
                    VRTK_SDK_Bridge.GetControllerHairlineDelta(SDK_BaseController.ButtonTypes.TriggerHairline,
                        controllerReference);
                hairGripDelta = VRTK_SDK_Bridge.GetControllerHairlineDelta(SDK_BaseController.ButtonTypes.GripHairline,
                    controllerReference);
