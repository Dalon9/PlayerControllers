...
namespace VRTK
{
    ...
    public class VRTK_BezierPointerRenderer : VRTK_BasePointerRenderer
    {
        ...
        public string PointerObjectLayer = "Default";
        ...
        protected virtual void CreateTracer()
        {
            ...
            //Custom code
            actualTracer.Create(tracerDensity, cursorRadius, customTracer, rescaleTracer, PointerObjectLayer);
        }
        ...
        protected virtual void CreateCursorLocations()
        {
            if (validLocationObject != null)
            {
                ...
                //Custom code
                if (PointerObjectLayer == "Default" || LayerMask.NameToLayer(PointerObjectLayer) < 0)
                    actualValidLocationObject.layer = LayerMask.NameToLayer("Ignore Raycast");
                else
                    actualValidLocationObject.layer = LayerMask.NameToLayer(PointerObjectLayer);
            ...
            if (invalidLocationObject != null)
            {
                ...
                if (PointerObjectLayer == "Default" || LayerMask.NameToLayer(PointerObjectLayer) < 0)
                    ...
                    actualInvalidLocationObject.layer = LayerMask.NameToLayer(PointerObjectLayer);
        ...
        protected virtual void CreateCursor()
        {
            ...
            if (PointerObjectLayer == "Default" || LayerMask.NameToLayer(PointerObjectLayer) < 0)
                ...
                actualCursor.layer = LayerMask.NameToLayer(PointerObjectLayer);
        ...
        protected virtual Vector3 ProjectForwardBeam()
        {
            ...
            bool hasRayHit = VRTK_CustomRaycast.Raycast(customRaycast, pointerRaycast, out collidedWith, PointerObjectLayer == "Default" || LayerMask.NameToLayer(PointerObjectLayer) < 0 ? (int)defaultIgnoreLayer : LayerMask.GetMask(PointerObjectLayer), calculatedLength);
        ...
        protected virtual Vector3 ProjectDownBeam(Vector3 jointPosition)
        {
            ...
            bool downRayHit = VRTK_CustomRaycast.Raycast(customRaycast, projectedBeamDownRaycast, out collidedWith, PointerObjectLayer == "Default" || LayerMask.NameToLayer(PointerObjectLayer) < 0 ? (int)defaultIgnoreLayer : LayerMask.GetMask(PointerObjectLayer), maximumLength.y);
        ...
        protected virtual void AdjustForEarlyCollisions(Vector3 jointPosition, Vector3 downPosition)
        {
            ...
            if (collisionCheckFrequency > 0 && actualTracer != null)
            {
                ...
                for (int i = 0; i < tracerDensity - checkFrequency; i += checkFrequency)
                {
                    ...
                    if (VRTK_CustomRaycast.Raycast(customRaycast, checkCollisionRay, out checkCollisionHit, PointerObjectLayer == "Default" || LayerMask.NameToLayer(PointerObjectLayer) < 0 ? (int)defaultIgnoreLayer : LayerMask.GetMask(PointerObjectLayer), nextPointDistance))
                    {
                        ...
                        if (VRTK_CustomRaycast.Raycast(customRaycast, downwardCheckRay, out downwardCheckHit, PointerObjectLayer == "Default" || LayerMask.NameToLayer(PointerObjectLayer) < 0 ? (int)defaultIgnoreLayer : LayerMask.GetMask(PointerObjectLayer), float.PositiveInfinity))
                        {
