...
namespace VRTK
{
    ...
    public class VRTK_CurveGenerator : MonoBehaviour
    {
        ...
        //Custom code (layer)
        public virtual void Create(int setFrequency, float radius, GameObject tracer, bool rescaleTracer = false, string layer = "Default")
        {
            ...
            if (customLineRenderer == null || lineRendererAndItem)
            {
                ...
                for (int f = 0; f < items.Length; f++)
                {
                    ...
                    //Custom code
                    if (layer == "Default" || LayerMask.NameToLayer(layer) < 0)
                        items[f].layer = LayerMask.NameToLayer("Ignore Raycast");
                    else
                        items[f].layer = LayerMask.NameToLayer(layer);
