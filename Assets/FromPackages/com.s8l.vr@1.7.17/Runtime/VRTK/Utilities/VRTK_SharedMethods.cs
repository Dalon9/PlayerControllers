...
namespace VRTK
{
    ...
    public static class VRTK_SharedMethods
    {
        ...
        public static Collider[] GetCollidersInGameObjects(GameObject[] gameObjects, bool searchChildren,
            bool includeInactive)
        {
            HashSet<Collider> foundColliders = new HashSet<Collider>();
            for (int i = 0; i < gameObjects.Length; i++)
            {
                Collider[] gameObjectColliders = (searchChildren
                    ? gameObjects[i].GetComponentsInChildren<Collider>(includeInactive)
                    : gameObjects[i].GetComponents<Collider>());
        ...
        public static GameObject FindEvenInactiveGameObject<T>(string gameObjectName = null,
            bool searchAllScenes = false) where T : Component
        {
        ...
        public static bool Vector2ShallowCompare(Vector2 vectorA, Vector2 vectorB, int compareFidelity)
        {
            ...
            return (Math.Round(Mathf.Abs(distanceVector.x), compareFidelity, MidpointRounding.AwayFromZero) <
                    float.Epsilon &&
                    Math.Round(Mathf.Abs(distanceVector.y), compareFidelity, MidpointRounding.AwayFromZero) <
                    float.Epsilon);
        }
        ...
        {
            Vector3[] worldDirections = (givenTransform != null
                ? new Vector3[] { givenTransform.right, givenTransform.up, givenTransform.forward }
        ...
        public static BuildTargetGroup[] GetValidBuildTargetGroups()
        {
            return Enum.GetValues(typeof(BuildTargetGroup)).Cast<BuildTargetGroup>().Where(group =>
            {
                ...
                FieldInfo targetGroupFieldInfo =
                    typeof(BuildTargetGroup).GetField(targetGroupName, BindingFlags.Public | BindingFlags.Static);
                bool validReturn = (targetGroupFieldInfo != null &&
                                    targetGroupFieldInfo.GetCustomAttributes(typeof(ObsoleteAttribute), false).Length ==
                                    0);
#if UNITY_WSA
        ...
        /// <returns></returns>
        private static IEnumerable<T> FindEvenInactiveComponentsInValidScenes<T>(bool searchAllScenes,
            bool stopOnMatch = false) where T : Component
        {
            ...
            if (searchAllScenes)
            {
                ...
                {
                    allSceneResults.AddRange(
                        FindEvenInactiveComponentsInScene<T>(SceneManager.GetSceneAt(sceneIndex), stopOnMatch));
                }
        ...
        private static IEnumerable<T> FindComponentInDdol<T>() where T : Component
        {
            var ddol = GetDdolObjects();
            var results = ddol.Where(x => x.GetComponentsInChildren<T>(true) != null);
            return results.SelectMany(x => x.GetComponentsInChildren<T>());
        }

        private static GameObject[] GetDdolObjects()
        {
