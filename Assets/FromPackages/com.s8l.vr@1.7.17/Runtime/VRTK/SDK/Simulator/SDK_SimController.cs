...
namespace VRTK
{
    ...
    public class SDK_SimController : SDK_BaseController
    {
        ...
        public override GameObject GetControllerByIndex(uint index, bool actual = false)
        {
            ...
            {
                case 1:
                    return (sdkManager != null && !actual
                        ? sdkManager.scriptAliasRightController
        ...
        {
            return (controllerReference != null && controllerReference.actual != null
                ? controllerReference.actual.transform
                : null);
        }
        ...
        [System.Obsolete(
            "GenerateControllerPointerOrigin has been deprecated and will be removed in a future version of VRTK.")]
            ...
            if (simPlayer != null)
            {
                switch (hand)
                {
                    case ControllerHand.Left:
                        model = simPlayer.transform.Find(string.Format("{0}/Hand", LEFT_HAND_CONTROLLER_NAME))
                            .gameObject;
                        break;
                    case ControllerHand.Right:
                        model = simPlayer.transform.Find(string.Format("{0}/Hand", RIGHT_HAND_CONTROLLER_NAME))
                            .gameObject;
                        break;
                }
            }
        ...
        {
            return (!isTouched ||
                    VRTK_SharedMethods.Vector2ShallowCompare(currentAxisValues, previousAxisValues, compareFidelity));
        }
        ...
        {
            if (buttonType == ButtonTypes.Trigger)
            {
                if (GetControllerButtonState(ButtonTypes.Trigger, ButtonPressTypes.PressDown, controllerReference))
                {
                    return Vector3.one;
                }
            }
        ...
        protected virtual void SetupPlayer()
        {
            if (rightController == null || leftController == null)
            {
                ...
                if (simPlayer != null)
                {
                    rightController = (rightController == null
                        ? simPlayer.transform.Find(RIGHT_HAND_CONTROLLER_NAME).GetComponent<SDK_ControllerSim>()
                        : rightController);
                    leftController = (leftController == null
                        ? simPlayer.transform.Find(LEFT_HAND_CONTROLLER_NAME).GetComponent<SDK_ControllerSim>()
                        : leftController);
                }
            }
        }
