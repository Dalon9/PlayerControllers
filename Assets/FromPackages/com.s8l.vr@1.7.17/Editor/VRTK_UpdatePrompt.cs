...
namespace VRTK.Editor
{
    ...
    public sealed class VRTK_UpdatePrompt : EditorWindow
    {
        ...
        private sealed class LatestRelease
        {
            ...
            [NonSerialized] public DateTime publishedDateTime;
            [NonSerialized] public List<string> changelogPages;
...
#pragma warning restore 649
            ...
            public static LatestRelease CreateFromJSON(string json)
            {
                ...
                latestRelease.changelogPages =
                    new List<string>((int) Mathf.Ceil(changelog.Length / (float) textLengthLimit));
                            ...
                            if (latestRelease.changelogPages.Count > 0)
                            {
                                using (new EditorGUILayout.HorizontalScope())
                                {
                                    ...
                                    using (new EditorGUI.DisabledGroupScope(
                                        changelogPageIndex == latestRelease.changelogPages.Count - 1))
                                    {
                                        if (GUILayout.Button("Next Page"))
                                        {
                                            changelogPageIndex = Math.Min(latestRelease.changelogPages.Count - 1,
                                                ++changelogPageIndex);
                                            changelogScrollPosition = Vector3.zero;
                                        }
                                    }
